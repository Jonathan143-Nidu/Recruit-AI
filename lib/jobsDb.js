...
export async function readJobs() {
    try {
        ...
        let data;
        if (res.data instanceof Readable || (res.data && typeof res.data.on === 'function')) {
            data = await new Promise((resolve, reject) => {
                let chunks = '';
                res.data.on('data', chunk => chunks += chunk);
                res.data.on('end', () => resolve(chunks));
                res.data.on('error', reject);
            });
            try {
                data = JSON.parse(data);
            } catch (parseErr) {
                console.error("Failed to parse stream data:", data.substring(0, 100));
                return [];
            }
